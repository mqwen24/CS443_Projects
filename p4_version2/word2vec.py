...
class Skipgram:
    ...
    def multi_hot(self, y_ind_list, num_classes):
        ...
        # print(len(y_ind_list))
        for i in range(len(y_ind_list)):
            # print(i, y_ind_list[i])
            if len(y_ind_list[i]) > 0:
    ...
    def plot_train_loss(self, train_loss, title="Training Loss for Skip-gram", n_samps=None, batch_sz=None):
        x_train = []
        ...
        axes.legend(loc="lower left", bbox_to_anchor=(0.6, 0.2), frameon=True, edgecolor="black")

        axes.set_title(title)
        plt.savefig("img/skip_gram_loss.svg")
        plt.show()